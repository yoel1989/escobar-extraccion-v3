...
async def obtener_todos_los_mensajes(fecha_inicio, fecha_fin):
    ...
    try:
        ...
        print(f"Buscando TODOS los mensajes del {fecha_inicio} al {fecha_fin}...")
        
        todos_los_mensajes = []
        mensajes_procesados = []
        codigos_vistos = set()  # Para detectar códigos duplicados
...
# PROCESAR TODOS los demás mensajes sin importar el formato
                    ...
                    if datos['fecha'] != "SIN FECHA":
                        try:
                            ...
                            if inicio.date() <= fecha_mensaje.date() <= fin.date():
                                ...
                                if datos['codigo'] not in codigos_vistos:
                                    mensajes_procesados.append(datos)
                                    todos_los_mensajes.append(mensaje_texto)
                                    codigos_vistos.add(datos['codigo'])
                                else:
                                    print(f"Código duplicado descartado: {datos['codigo']}")
                        except:
        ...
        print(f"Total de mensajes encontrados: {len(todos_los_mensajes)}")
        print(f"Mensajes procesados (excluyendo 'Desconectado' y duplicados): {len(mensajes_procesados)}")
        print(f"Códigos duplicados descartados: {len(todos_los_mensajes) - len(mensajes_procesados)}")
...
def calcular_ventas_y_comisiones(datos):
    ...
    total_valor = 0
    total_comision = 0
    cliente_valor = 45000  # Valor fijo de cliente
    cliente_comision = 5000  # Comisión fija de cliente
    television_valor = 25000  # Valor fijo de televisión (sin comisión)
    ...
    for item in datos:
        ...
        elif tiempo == "02:00:00" or tiempo.startswith("02:"):
            tipo_pin = "2Horas" 
            precio = PRECIOS_PINES["2Horas"]
            comision = COMISIONES_POR_PIN["2Horas"]
        elif tiempo == "03:00:00" or tiempo.startswith("03:"):
            tipo_pin = "3Horas"
            precio = PRECIOS_PINES["3Horas"]
            comision = COMISIONES_POR_PIN["3Horas"]
...
# Calcular comisión por alcanzar metas
    ...
    # Sumar cliente y televisión al total
    total_con_adicionales = total_valor + cliente_valor + television_valor
    total_comision_con_cliente = total_comision + cliente_comision
    total_a_enviar = total_con_adicionales - total_comision_con_cliente - comision_alcanza
    ...
    return {
        ...
        'television_valor': television_valor,
        'total_con_adicionales': total_con_adicionales,
        'cliente_comision': cliente_comision,
        'total_comision_con_cliente': total_comision_con_cliente,
...
def crear_tabla_resumen(ventas_calculadas, fecha_inicio, fecha_fin, nombre_sitio):
  ...
  # Tipos de PIN en el orden específico
    tipos_pin_orden = ["1Hora", "2Horas", "3Horas", "1Dia", "30Dias"]
...
# Cliente
    tabla_resumen.append(['', '', 'CLIENTE', f"$ {ventas_calculadas['cliente_valor']:,}"])
    
    # Television
    tabla_resumen.append(['', '', 'TELEVISION', f"$ {ventas_calculadas['television_valor']:,}"])
...
async def main():
    """Función principal"""
    print("Exportador de Mensajes de Telegram - TODOS los mensajes")
    print("-" * 60)
    ...
    if mensajes:
        ...
        print(f"Sitio identificado: {nombre_sitio}")
        print(f"\nRESUMEN FINANCIERO:")
        print("=" * 60)
        for tipo_pin in ["1Hora", "2Horas", "3Horas", "1Dia", "30Dias"]:
        ...
        print(f"   CLIENTE: ${ventas['cliente_valor']:,}")
        print(f"   TELEVISION: ${ventas['television_valor']:,}")
        print(f"   TOTAL CON ADICIONALES: ${ventas['total_con_adicionales']:,}")
        print(f"   COMISION CLIENTE: ${ventas['cliente_comision']:,}")
        print(f"   TOTAL COMISIONES: ${ventas['total_comision_con_cliente']:,}")
        print(f"   COMISIÓN ALCANZA: ${ventas['comision_alcanza']:,}")
        print(f"   TOTAL A ENVIAR: ${ventas['total_a_enviar']:,}")
        print("=" * 60)
        print(f"   DETALLE: {len(mensajes)} registros procesados")

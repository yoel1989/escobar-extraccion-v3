...
function initializeApp() {
    ...
    // No cargar historial automáticamente, esperar a que el usuario lo solicite
    updateStatus();
    startAutoRefresh();
}
...
async function triggerExtraction(tipo, puntos, startDate, endDate) {
    ...
    // Crear registro en la base de datos - SIN 'creado_at' (lo pone Supabase)
    ...
    try {
        console.log('Enviando datos:', extraccionData);
        ...
        console.log('Extracción creada:', extraccion);
        
        // Disparar GitHub Action
        await triggerGitHubAction(extraccion.id, puntos, startDate, endDate);
        
        showToast(`Extracción ${tipo} iniciada correctamente - ID: ${extraccion.id}`, 'success');
        
        // Esperar un momento y recargar historial
        setTimeout(() => {
            loadHistory();
        }, 1000);
        ...
        showToast(`Error iniciando extracción: ${error.message}`, 'error');
    } finally {
...
async function triggerGitHubAction(extraccionId, puntos, startDate, endDate) {
    ...
    // Por ahora, simulamos el trigger y mostramos mensaje
    
    try {
        // Para desarrollo, simulamos el trigger
        console.log('GitHub Action triggered:', { extraccionId, puntos, startDate, endDate });
        
        // Mostrar mensaje de que se está procesando en GitHub Actions
        showToast('Extracción en cola - Procesando en GitHub Actions...', 'info');
        
        // Simular que el workflow se está ejecutando
        setTimeout(() => {
...
async function updateStatus() {
    try {
        ...
        // Extracciones totales para contar
        const totalResponse = await fetch(`${SUPABASE_URL}/rest/v1/extracciones?limit=100`, {
            headers: {
                'apikey': SUPABASE_KEY,
                'Authorization': `Bearer ${SUPABASE_KEY}`
            }
        ...
        if (totalResponse.ok) {
            const extracciones = await totalResponse.json();
            const today = new Date().toISOString().split('T')[0];
            const todayCount = extracciones.filter(e => 
                e.creado_at && e.creado_at.startsWith(today)
            ).length;
            
            document.getElementById('todayExtractions').textContent = todayCount;
            console.log(`Estado actualizado: ${todayCount} extracciones hoy`);
        }
...
function loadAvailableDownloads() {
    // Cargar archivos ZIP disponibles de GitHub Actions artifacts
    const downloadsList = document.getElementById('downloadsList');
    if (downloadsList) {
        downloadsList.innerHTML = '<p><i class="fas fa-spinner fa-spin"></i> Cargando archivos disponibles...</p>';
        
        // Simular búsqueda de archivos (en producción esto consultaría GitHub API)
        setTimeout(() => {
            downloadsList.innerHTML = `
                <div class="download-item" style="padding: 1rem; background: var(--light-bg); border-radius: 8px;">
                    <h4><i class="fas fa-file-archive"></i> Resultados Disponibles</h4>
                    <p>Los archivos ZIP se generan cuando se completan las extracciones.</p>
                    <p>Una vez completada una extracción, podrás descargar los resultados desde aquí.</p>
                </div>
            `;
        }, 1000);
    }
...
function downloadLatestResults() {
    showToast('Buscando resultados más recientes...', 'info');
    
    // En producción, esto descargaría desde GitHub API
    // Por ahora, simulamos
    setTimeout(() => {
        showToast('Los resultados estarán disponibles cuando se complete la extracción actual', 'success');
    }, 1500);
}
